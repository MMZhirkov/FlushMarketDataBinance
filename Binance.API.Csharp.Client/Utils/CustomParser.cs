...
namespace Binance.API.Csharp.Client.Utils
{
    ...
    public class CustomParser
    {
        ...
        public OrderBook GetParsedOrderBook(dynamic orderBookData)
        {
            ...
            {
                bids.Add(new OrderBookOffer() { Price = decimal.Parse(item[0].ToString().Replace(".",",")), Quantity = decimal.Parse(item[1].ToString().Replace(".", ",")) });
            }

            foreach (JToken item in ((JArray)orderBookData.asks).ToArray())
            {
                asks.Add(new OrderBookOffer() { Price = decimal.Parse(item[0].ToString().Replace(".", ",")), Quantity = decimal.Parse(item[1].ToString().Replace(".", ",")) });
            }
